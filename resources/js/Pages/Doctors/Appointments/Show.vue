    ...
    <app-layout title="Citas">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <div class="flex items-center gap-4">
                    <span class="underline cursor-pointer" @click="$inertia.visit(dashboardLink)">Citas</span>
        ...
        <app-body>
            <div class="mt-4 capitalize text-center text-xl">
                {{ appointment.status_label }} - <span v-if="patient.is_new" style="color:green"> {{ patient.is_new }}
                </span>
                <br>
            ...
            <template v-else>
                <div class="border rounded p-3 text-center">
                    <a href="https://wa.me/51996726834" style="text-decoration: underline; ">
                        Hay {{ rates.length }} tarifas con este paciente, pero el Doctor no cuenta con las subfamilias,
                        ponerse en contacto con el gerente para hacerle saber
                    </a>
                </div>
            </template>

            <div class="mt-8 text-center flex flex-wrap gap-4 justify-center">
                <front-button color="yellow" v-if="role !== 'assistant'"
                    @click="$inertia.visit(route('patients.historygroup.index', appointment.patient_id))">
                    Ver Historial Clínico
                </front-button>

                <front-button color="red" v-if="appointment.status == 1 && role !== 'assistant' && role !== 'doctor'"
                ...
                <front-button color="green" @click="markAssisted"
                    v-show="appointment.status != 4 && appointment.status != 3 && role !== 'doctor' && currentDate() == appointment.date">
                    Marcar Asistencia
                </front-button>
        ...
        >
          Generar Link de Pago
            ...
            <div class="mt-8 text-center flex flex-wrap gap-4 justify-center">
                ...
                <front-button color="yellow" v-show="appointment.status != 4 && appointment.status != 3"
                    @click="$inertia.visit(route('reschedule.pickOffice', appointment.id))">
                    Reprogramar Cita
                </front-button>

                <front-button v-if="role == 'admin' || role == 'assistant'" color=""
        ...
        markAssisted() {
            if (this.rate) {
                if (this.rate.can_assist) {
                    ...
                    const url = route('patients.rates.payApp', [this.rate.id, this.appointment.id]);
                    this.$inertia.visit(url);
                }
            } else {
                const url = route('doctors.appointments.rates.index', this.appointment.id);
                this.$inertia.visit(url);
            }
        },
        markNotAssisted() {
